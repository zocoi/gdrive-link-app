...
export const Flex: FC<AppProps> = (props: AppProps) => {
  ...
  return (
    <Root>
      <div className={`${getPaddingClass()} rounded-linktree bg-linktree-primary`}>
        <div className={`grid ${getGridClass()} ${getGapClass()}`}>
          {stats.map((stat) => (
            <div key={`${stat.name}-${stat.value}`} className="flex flex-col justify-center text-center">
              <div className={`${textSizes.value} font-bold leading-none tracking-tight text-linktree-button-text`}>
                {formatValue(stat.value)}
              </div>
              <div className={`${textSizes.name} mt-1.5 font-normal leading-tight text-linktree-button-text/70`}>
                {stat.name}
              </div>
            </div>
          ))}
